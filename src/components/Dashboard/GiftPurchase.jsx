...
// ✅ Placeholder for empty transaction list
const TransactionPlaceholder = ({ title, icon }) => (
  <div className="flex flex-col items-center justify-center h-full text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200">
    ...
    <p className="text-slate-500 max-w-md italic">
      No distribution or debit records found for this period.
    </p>
  </div>
);
...
export default function AdminDistributions() {
  ...
  // Use dynamic API base for localhost/dev/production
  const API_BASE = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000/api';

  // Fetch all transactions
  const fetchTransactions = async () => {
    try {
      const res = await axios.get(`${API_BASE}/wallet/admin/transactions`);
      if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(`${API_BASE}/wallet/admin/transactions/${id}`);
      if (res.data.success) {
  ...
  const applyFilter = () => {
    ...
    let filtered = transactions.filter((tx) => tx.category === 'GIFT_PURCHASE');
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap items-center gap-6 mb-6 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex flex-col">
            <label className="text-[10px] font-bold text-slate-400 uppercase ml-1 mb-1">
              Timeframe
            </label>
            <select
              value={filterType}
          ...
          {filterType === 'range' && (
            <div className="flex items-center gap-3">
              <input
                ...
                className="bg-slate-50 border border-slate-200 text-sm rounded-xl p-2.5 font-bold outline-none"
              />
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredTransactions.map((tx) => (
                    <tr key={tx._id} className="hover:bg-rose-50/30 transition-colors">
                      ...
                      <td className="p-4">
                        <span className="font-mono font-black text-rose-600">🪙 {tx.coins}</span>
                      </td>
